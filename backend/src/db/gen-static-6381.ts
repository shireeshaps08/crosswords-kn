...
// Black cells — corrected from printed grid image + slot constraints
...
// Confirmed answers from PDF footer of puzzle 6382 (N-1 offset rule)
const clues = {
  across: [
    ...
    { number:7,  clue:'ಸ್ವಾಭಾವಿಕ ವಾದುದು ಸಜದಲ್ಲಿದೆ!',         answer:'ಸಹಜ',       direction:'across', row:2, col:5, length:4 },
...
// Build solution grid (letters placed from answers)
...
for (const row of grid) {
  console.log(row.map(c => c.blocked ? '■' : (c.number ?? '·')).join(' '));
}
console.log('\nSolution row 0:', solution[0].map(c => c.blocked ? '■' : (c.letter || '_')).join('|'));
